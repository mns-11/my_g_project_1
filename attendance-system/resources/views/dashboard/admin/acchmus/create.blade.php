...
@section('title', ucwords(__('main.add_new_course')))
...
@section('content')
    ...
    <div class="content p-4">
        <div class="card card-body">
            <div class="container">
                <h2 class="mt-4">{{ucwords(__('main.add_new_course'))}}</h2>
                <form action="{{ route('acchmus.store') }}" method="POST">
                    @csrf
                    <div class="row">
...
{{--                        </div>--}}
                        ...
                        <div class="col-md-12">
                            ...
                            <select class="form-select @error('hall_id') is-invalid @enderror" id="hallSelect" name="hall_id">
                                ...
                                @foreach($halls as $hall)
                                    <option
                                        value="{{ $hall->id }}" {{ (old('hall_id') == $hall->id) ? 'selected' : '' }}>{{ $hall->name }}</option>
                                @endforeach
                            </select>
                            @error('hall_id')
                            <div class="invalid-feedback">{{ $message }}</div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            {{--    const filteredCourses = allCourses.filter(course => course.college_id == selectedCollegeId && course.major_id == selectedMajorId);--}}
            ...
            {{--            courseSelect.value = oldCourseId; // ✅ Only set if option exists--}}
            {{--        }--}}
            {{--    }--}}
            {{--}--}}
