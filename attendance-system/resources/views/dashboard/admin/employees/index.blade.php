...
@section('title', ucwords(__('main.employee_management')))
...
@section('content')
    ...
    <div class="content p-4">
        <div class="header-container">
            <h2>{{ ucwords(__('main.employee_management'))}}</h2>
            <div class="d-flex flex-wrap gap-2">
                <form class="position-relative" method="GET" action="{{route('employees.index') }}" id="search-form">
                ...
                <a href="{{route('employees.create')}}" class="btn btn-primary" >
                    <i class="fas fa-plus me-1"></i>{{ucwords(__('main.add_employee'))}}
                </a>
            </div>
        </div>
        ...
        <div class="table-container">
            ...
            <tr>
                ...
                <th style="background-color:#218838 ;color: white;">{{ucfirst(__('main.role'))}}</th>
            ...
            @foreach($users as $user)
                <tr>
                    ...
                    <td>{{$user->name}}</td>
                    <td><span class="role-badge">{{$user->roles->first()->name}}</span></td>
                    ...
                    <td>{{$user->phone}}</td>
                    <td>
                        <a href="{{route('employees.edit', $user->id)}}" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                        @if(auth()->id() != $user->id)
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title">{{ucwords(__('main.confirm_employee_deletion'))}}</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                        </div>
                                        <div class="modal-body">
                                            <div class="text-center mb-4">
                                                <i class="fas fa-exclamation-triangle fa-3x text-danger mb-3"></i>
                                                <h4>{{ucwords(__('main.are_you_sure_you_want_to_delete_employee_data'))}}</h4>
                                                <p class="mt-3">{{ucwords(__('main.all_employee_data_will_be_deleted_and_this_action_cannot_be_undone'))}}</p>
                                            </div>
                                            <div class="mb-3">
