...
@section('title', ucwords(__('main.hall_management')))
...
@section('content')
    ...
    <div class="content p-4">
        <div class="header-container">
            <h2>{{ ucwords(__('main.hall_management'))}}</h2>
            <div class="d-flex flex-wrap gap-2">
                <form class="position-relative" method="GET" action="{{route('halls.index') }}" id="search-form">
                ...
                <a href="{{route('halls.create')}}" class="btn btn-primary" >
                    <i class="fas fa-plus me-1"></i>{{ucwords(__('main.add_hall'))}}
                </a>
            </div>
        </div>
        ...
        <div class="table-container">
            <table class="table table-striped" id="staff-table">
                ...
                @foreach($halls as $hall)
                    <tr>
                        ...
                        <td>{{$hall->location}}</td>
                        <td>
                            <a href="{{route('halls.edit', $hall->id)}}" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                            <a type="button" class="btn btn-sm btn-danger " data-bs-title="{{ucfirst(__('main.delete'))}}" data-bs-toggle="modal" data-bs-target="#deleteStaffModal-{{$hall->id}}"><i class="fas fa-trash"></i></a>

                            <div class="modal fade" id="deleteStaffModal-{{$hall->id}}" tabindex="-1">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title">{{ucwords(__('main.confirm_hall_deletion'))}}</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                        </div>
                                        <div class="modal-body">
                                            <div class="text-center mb-4">
                                                <i class="fas fa-exclamation-triangle fa-3x text-danger mb-3"></i>
                                                <h4>{{ucwords(__('main.are_you_sure_you_want_to_delete_this_hall'))}}</h4>
                                            </div>
                                            <div class="mb-3">
            ...
            @if ($halls instanceof \Illuminate\Pagination\LengthAwarePaginator)
                {{ $halls->appends(['search' => request()->query('search')])->links('pagination::bootstrap-4') }}
            @endif
        </div>
