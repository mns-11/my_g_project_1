...
@section('title', ucwords(__('main.add_course')))
...
@section('content')
    ...
    <div class="content p-4">
        <div class="card card-body">
            <div class="container">
                <h2 class="mt-4">{{ucwords(__('main.add_course'))}}</h2>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="type" class="form-control @error('type') is-invalid @enderror" id="Type">
                                ...
                                <option
                                    value="{{ App\Enums\CourseType::PRACTICAL->value }}" {{old('type') === App\Enums\CourseType::PRACTICAL->value ? 'selected' : '' }}> {{ __('main.' . App\Enums\CourseType::PRACTICAL->getName())}} </option>
                                <option
                                    value="{{ App\Enums\CourseType::THEORETICAL->value }}" {{old('type') === App\Enums\CourseType::THEORETICAL->value ? 'selected' : '' }}> {{ __('main.' . App\Enums\CourseType::THEORETICAL->getName())}} </option>
                            </select>
                            @error('type')
                            <span class="validation-text text-danger">{{ $message }}</span>
                            @enderror
...
{{--                        </div>--}}
                        ...
                        <div class="col-md-6">
                            <label for="Type" class="col-form-label">{{ucwords(__('main.term'))}}</label>
                            <select name="term" class="form-control @error('term') is-invalid @enderror" id="Type">
                                <option selected disabled value="">{{ucwords(__('main.select_term'))}}</option>
                                <option
                                    value="{{ App\Enums\AcademicTerm::ONE->value }}" {{old('term') === App\Enums\AcademicTerm::ONE->value ? 'selected' : '' }}> {{ __('main.' . strtolower(App\Enums\AcademicTerm::ONE->getName()))}} </option>
                                <option
                                    value="{{ App\Enums\AcademicTerm::TWO->value }}" {{old('term') === App\Enums\AcademicTerm::TWO->value ? 'selected' : '' }}> {{ __('main.' . strtolower(App\Enums\AcademicTerm::TWO->getName()))}} </option>
                            </select>
                            @error('term')
                            <span class="validation-text text-danger">{{ $message }}</span>
                            @enderror
