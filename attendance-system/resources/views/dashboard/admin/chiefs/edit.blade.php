...
@section('title', ucwords(__('main.edit_chief')))
...
@section('content')
    ...
    <div class="content p-4">
        <div class="card card-body">
            <div class="container">
                <h2 class="mt-4">{{ucwords(__('main.edit_chief'))}}</h2>
                <form action="{{ route('chiefs.update', $user->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('major_id') is-invalid @enderror"
                                    id="validationCustom04" required name="major_id">
                                <option selected disabled value="">{{ucwords(__('main.select_major'))}}</option>
                                @foreach($majors as $major)
                                    <option
                                        value="{{ $major->id }}" {{ (old('major_id', $user->major_id) == $major->id) ? 'selected' : '' }}>{{ $major->name }}</option>
                                @endforeach
                            </select>
                            @error('major_id')
                    ...
                    <div class="col-12">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <a  href="{{route('chiefs.index')}}" type="button" class="btn bg-danger-subtle text-danger">{{ucfirst(__('main.cancel'))}}</a>
                        </div>
                    </div>
