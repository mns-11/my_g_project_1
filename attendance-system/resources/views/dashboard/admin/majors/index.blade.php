...
@section('title', ucwords(__('main.major_management')))
...
@section('content')
    ...
    <div class="content p-4">
        <div class="header-container">
            <h2>{{ ucwords(__('main.major_management'))}}</h2>
            <div class="d-flex flex-wrap gap-2">
                <form class="position-relative" method="GET" action="{{route('majors.index') }}" id="search-form">
                ...
                <a href="{{route('majors.create')}}" class="btn btn-primary" >
                    <i class="fas fa-plus me-1"></i>{{ucwords(__('main.add_major'))}}
                </a>
            </div>
        </div>
        ...
        <div class="table-container">
            ...
            <tr>
                ...
                <th style="background-color:#218838 ;color: white;">{{ucwords(__('main.levels'))}}</th>
                <th style="background-color:#218838 ;color: white;">{{ucwords(__('main.major_location'))}} </th>
            ...
            @foreach($majors as $major)
                <tr>
                    ...
                    <td>{{$major->num_levels}}</td>
                    <td>{{$major->location}}</td>
                    <td>{{$major->phone}}</td>
                    <td>{{$major->email}}</td>
                    <td>
                        <a href="{{route('majors.edit', $major->id)}}" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                        <a type="button" class="btn btn-sm btn-danger " data-bs-title="{{ucfirst(__('main.delete'))}}" data-bs-toggle="modal" data-bs-target="#deleteStaffModal-{{$major->id}}"><i class="fas fa-trash"></i></a>

                        <div class="modal fade" id="deleteStaffModal-{{$major->id}}" tabindex="-1">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">{{ucwords(__('main.confirm_major_deletion'))}}</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                    </div>
                                    <div class="modal-body">
                                        <div class="text-center mb-4">
                                            <i class="fas fa-exclamation-triangle fa-3x text-danger mb-3"></i>
                                            <h4>{{ucwords(__('main.are_you_sure_you_want_to_delete_this_major'))}}</h4>
                                        </div>
                                        <div class="mb-3">
        ...
        @if ($majors instanceof \Illuminate\Pagination\LengthAwarePaginator)
            {{ $majors->appends(['search' => request()->query('search')])->links('pagination::bootstrap-4') }}
        @endif
    </div>
