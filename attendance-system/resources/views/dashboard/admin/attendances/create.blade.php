...
@section('title', ucwords(__('main.record_new_attendance')))
...
@section('content')
    ...
    <div class="content p-4">
        <div class="card card-body">
            <div class="container">
                <h2 class="mt-4">{{ucwords(__('main.record_new_attendance'))}}</h2>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="status" class="form-control @error('status') is-invalid @enderror" id="status">
                                ...
                                <option
                                    value="{{ App\Enums\AttendanceStatus::PRESENT }}" {{old('status') === App\Enums\AttendanceStatus::PRESENT->value ? 'selected' : '' }}> {{__('main.' . App\Enums\AttendanceStatus::PRESENT->getName())}} </option>
                                <option
                                    value="{{ App\Enums\AttendanceStatus::ABSENT }}" {{old('status') === App\Enums\AttendanceStatus::ABSENT->value ? 'selected' : '' }}> {{__('main.' . App\Enums\AttendanceStatus::ABSENT->getName())}} </option>
                            </select>
                            @error('status')
