...
@section('title',ucwords(__('main.edit_lecture')))
        ...
        <div class="row animate__animated animate__fadeIn">
            <div class="col-md-8 mx-auto">
                <div class="card p-4" style="background: linear-gradient(135deg, #f5f6fa9f 0%, #e0e6f0a6 100%);">
                    <h3 class="text-center mb-4"><strong style="color: #228B22;">{{ucwords(__('main.edit_lecture'))}}</strong></h3>
                    <form id="lectureForm" action="{{route('lectures.update',$lecture->id)}}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('course_id') is-invalid @enderror" id="course"  style="color: #FFA500;" name="course_id">
                                ...
                                @foreach($courses as $course)
                                    <option
                                        value="{{ $course->id }}" {{ (old('course_id',$lecture->course_id) == $course->id) ? 'selected' : '' }}>{{ $course->name }}</option>
                                @endforeach
                            </select>
                            @error('course_id')
                        ...
                        <div class="mb-3">
                            ...
                            <select name="type" class="form-control @error('type') is-invalid @enderror" id="Type">
                                ...
                                <option
                                    value="{{ App\Enums\CourseType::PRACTICAL }}" {{old('type',$lecture->type->value) === App\Enums\CourseType::PRACTICAL->value ? 'selected' : '' }}> {{ __('main.' . App\Enums\CourseType::PRACTICAL->getName())}} </option>
                                <option
                                    value="{{ App\Enums\CourseType::THEORETICAL }}" {{old('type', $lecture->type->value) === App\Enums\CourseType::THEORETICAL->value ? 'selected' : '' }}> {{ __('main.' . App\Enums\CourseType::THEORETICAL->getName())}} </option>
                            </select>
                            @error('type')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select class="form-select @error('college_id') is-invalid @enderror" id="collegeSelect" name="college_id">
                                    <option selected disabled value="">{{ ucwords(__('main.select_college')) }}</option>
                                    @foreach($colleges as $college)
                                        <option value="{{ $college->id }}" {{ old('college_id',$lecture->college_id) == $college->id ? 'selected' : '' }}>
                                            {{ $college->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" class="form-control @error('datetime') is-invalid @enderror" id="datetime" name="datetime" value="{{ old('datetime',$lecture->datetime) }}">
                            @error('datetime')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('duration') is-invalid @enderror" id="duration" placeholder=""  name="duration" value="{{old('duration', $lecture->duration)}}">
                            @error('duration')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
{{--                            <input type="text" class="form-control @error('hall') is-invalid @enderror" id="location" placeholder=""  name="hall" value="{{old('hall',$lecture->hall)}}">--}}
{{--                            @error('hall')--}}
{{--                            <div class="invalid-feedback">{{ $message }}</div>--}}
{{--                            @enderror--}}
{{--                        </div>--}}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateMajors() {
                ...
                const oldMajorId = "{{ old('major_id', $lecture->major_id) }}";
                if (oldMajorId) {
            ...
            function updateLevels() {
                ...
                if (!isNaN(numLevels) && numLevels > 0) {
                    ...
                    const oldLevelId = "{{ old('level_id', $lecture->level_id) }}";
                    if (oldLevelId) {
